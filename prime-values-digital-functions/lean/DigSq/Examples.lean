...
The paper's running examples: the power sums `a ↦ a^r` (Corollary 1.4), the
digit-occurrence counters (Corollary 1.5), and in particular
...
/-- Power weights satisfy the coprimality hypothesis (3), because `g(1) = 1`. -/
...
/-- The sum of the `r`-th powers of the base-`b` digits of `n` (Corollary 1.4). -/
...
/-! ### `d_g = 1` for every even power in base ten (§6.1)

For `g(a) = a^r` in base ten, §6.1 gives `d_g = 3` for odd `r ≥ 3` and `d_g = 1`
...
/-- **§6.1 for even powers in base ten:** `d_g = 1` for `g(a) = a^r`, `r` even. -/
...
/-! ### Digit-occurrence weights (Corollary 1.5) -/
...
/-- `digitCount` satisfies hypothesis (3): the value at `c` itself is `1`. -/
