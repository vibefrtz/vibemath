...
half of the paper (Theorem 1.10 and its companions) uses no axiom at all.
...
* The hypothesis `g ∈ ℱ₊` is `Weight b` together with `Weight.Coprime₁`: the
  ...
  the paper) — which is why `DSS/Weight.lean` sits above this file.
...
The result, as quoted by the paper's §3 (proof of Corollary 1.3):
