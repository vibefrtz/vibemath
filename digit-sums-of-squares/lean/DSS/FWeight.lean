...
`w₀, …, w_{b−1}`, the function `F_w(n) = ∑_{j<ℓ_b(n)} w_{ε_j(n)}` splits as
  ...
  `F_w(n) = w₀·ℓ_b(n) + g_w(n)`,     `g_w(a) = w_a − w₀`  (eq. (14)),

with `g_w` strongly `b`-additive.  This file formalises:

* the split (14), and the zero-digit counter `Z_b = F_{(1,0,…,0)}`;
* the parameters of Corollary 1.7: the strongly additive part of `Z_b` has
  `μ = −(b−1)/b`, `σ² = (b−1)/b²`, `d_g = 1`, and the target `Z_b(p) = r` on
  the shell of `br`-digit primes sits at the exact centre of the window;
* **Example 5.5** (base 3, weights `(1,2,−3)`): on odd shells the only prime
  value of `F_w` is `2` — the congruence forcing behind the numerics of §5.4;
* the base-4 mean-zero example of the introduction, with `d_g = 3`;
...
/-! ### The parameters of Corollary 1.7 -/
...
/-- **The admissible shell is exact** (Corollary 1.7): for the target
...
/-! ### Example 5.5: base 3, weights `(1, 2, −3)` -/
...
/-- **Example 5.5, the forcing**: on an odd shell every prime value of
