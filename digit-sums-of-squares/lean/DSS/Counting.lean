...
cut-off `x` (the paper's `x_k = b^{k/μ_g}` is irrational, so integer cut-offs
would force an avoidable floor-error analysis).
...
/-- `π_k(x) = #{p ≤ x : g(1)·p ≡ k (mod d_g)}` of Theorem 2.1. -/
...
This ties the two counting functions of Theorem 2.1 together: `π_k(x)` really is
