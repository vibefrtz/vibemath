...
Everything conditional in this development rests on the two axioms declared
in [`DSS/Cited.lean`](lean/DSS/Cited.lean): Corollary 4.6 (the `P₂` theorem
along the primes) and its two instances use `mmr` and `hhbr`; the square
implication `square_p2_of_llt` (Corollary 1.3, `P₂` part, derived from the
*statement* of Theorem 1.1) and its binary instance use `hhbr` **only** —
the square local limit theorem enters as a hypothesis, never as an axiom.
The value of the axioms is exactly the fidelity of the transcriptions; this
file records how each was checked.

The unconditional majority of the development, including the whole of
Theorem 1.8, does not depend on either axiom; the build-time guard
...
The accompanying paper (§4, proofs of Corollaries 4.6 and 1.3) uses the
result in the form:
...
Corollary 4.6 arises exactly by the paper's argument: `hhbr` manufactures
...
as the axiom guard certifies.  The square counterpart `square_p2_of_llt`
(`DSS/SquareP2.lean`) replaces `mmr` by the hypothesis `SquareLLT g` — the
transcribed statement of the paper's own Theorem 1.1 — and therefore
consumes `hhbr` alone; the guard certifies that too.
