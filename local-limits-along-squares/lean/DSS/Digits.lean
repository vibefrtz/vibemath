...
* block additivity: `s_b(x + b^k·m) = s_b(x) + s_b(m)` for `x < b^k`
  (digit sums add along a base-`b^k` expansion, eq. (77) of the paper);
* the complement rule of Lemma 7.6 of the paper (eq. (78) is its digit-sum
  instance at `c = 2a`): `s_b(b^k − c) + s_b(c−1) = k(b−1)`
  for `1 ≤ c ≤ b^k`;
...
`s_b(x + b^k·m) = s_b(x) + s_b(m)`.  This is the identity behind eq. (77)
...
sum; in particular `s_{10}((10n)²) = s_{10}(n²)`, the identity of §8.3. -/
theorem sb_pow_mul {b : ℕ} (hb : 2 ≤ b) (k m : ℕ) :
    sb b (b ^ k * m) = sb b m := by
  have h := sb_add_pow_mul hb k 0 m (by positivity)
  simpa using h

/-- **The complement rule** (Lemma 7.6 of the paper; eq. (78) is its digit-sum
instance at `c = 2a`): the base-`b` digits of `b^k − c`, for
