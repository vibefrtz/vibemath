...
`σ_g²`, `d_g` of the paper, and Lemma 2.2 of the predecessor paper
(`σ_g > 0` and `gcd(g(1), d_g) = 1` under primitivity).
...
/-- The coprimality hypothesis (1): `gcd(g(1), …, g(b-1)) = 1`. -/
...
/-- **The congruence (3) of the paper:** `d_g` divides `g(n) - g(1)·n`, so the
...
/-- **Lemma 2.2 of the predecessor, first part.** Under (1) the weight is not
constant on the digits, so `σ_g > 0`. -/
...
/-- **Lemma 2.2 of the predecessor, second part.** Under (1),
`gcd(g(1), d_g) = 1`. -/
theorem coprime_w_one_dg (hg : g.Coprime₁) :
  ...
  -- `d` divides every `g(a)`, hence divides the `gcd` of (1), which is `1`
