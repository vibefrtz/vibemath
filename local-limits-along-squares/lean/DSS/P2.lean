...
**Corollary 4.6 of the paper**: for `g` with `μ_g > 0` and `d_g = 1`,
...
/-- **Corollary 4.6**: for `μ_g > 0` and `d_g = 1` there is `c > 0` with
...
`≫ π(x)/log log x` primes**: the `S`-instance of Corollary 4.6, the
...
primes**: the `s_2`-instance of Corollary 4.6. -/
